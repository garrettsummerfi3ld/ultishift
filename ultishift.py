...
version = 'CReader-6'
pathList = [f'{path}/ultishift', f'{path}/ultishift/fileconvert', f'{path}/ultishift/fileconvert/encrypted',
            f'{path}/ultishift/fileconvert/decrypted', f'{path}/ultishift/config']
fileList = [f'{path}/ultishift/config/defaultconfig.yaml']
defaultData = ['"[please_insert_a_excessively_long_key]"', '"[please_insert_a_excessively_long_key]"', '"[please_insert_a_excessively_long_key]"',
               '11charsnumb', '32randommixedcharacterandnumbers', '"filekey[please_insert_a_excessively_long_key]"', '123456789123456789']
...
def fileintegrity():
    ...
    if os.path.isfile(f'{path}/defaultconfig.yaml') == True:
        if data['class'][0]['identification'] != version:
            print(
                '(FileIntegrity): Incorrect version!\n(FileIntegrity): Rebuilt the default config.')
            shutil.copyfile(f'{path}/defaultconfig.yaml',
                            f'{path}/defaultconfig.yaml.old')
    ...
    try:
        ...
        shutil.copyfile(f'{path}/defaultconfig.yaml',
                        f'{path}/defaultconfig.yaml.broken')
        with open(f'{path}/defaultconfig.yaml', 'w') as file:
            file.write(configfile)
        file.close()
    ...
    if cl['class'][0]['identification'] == version:
        ...
        try:
            if len(cl['desencrypt'][0]['desdata1']) != 11:
                ...
                print(
                    f'[ERROR] (ConfigLoader): \'desdata1\' has an invalid length. It must be 11 characters long.')
                raise ValueError
                ...
                desmatch = re.search(
                    r'^[^\W_]{3,48}$', cl['desencrypt'][0]['desdata1'])
                if desmatch == None:
                    if noexceptions == True:
                ...
                desmatch = re.search(
                    r'^[^\W_]{3,48}$', cl['desencrypt'][1]['desdata2'])
                if desmatch == None:
                    if noexceptions == True:
        ...
        try:
            if len(str(cl['discord'][1]['peerid'])) <= 17:
                print(
                    f'[WARNING] (ConfigLoader): \'peerid\' hasn\'t been set or is invalid.')
            elif len(str(cl['discord'][1]['peerid'])) == 18:
                lettersinid = re.search(
                    r'\d{18}', str(cl['discord'][1]['peerid']))
                if lettersinid == None:
                    print(
                        f'[WARNING] (ConfigLoader): \'peerid\' is an invalid Discord userid.')
            elif len(str(cl['discord'][1]['peerid'])) >= 18:
                lettersinid = re.search(
                    r'\d{18}', str(cl['discord'][1]['peerid']))
                if lettersinid == None:
        ...
        configData = [bigprivatekey, randomdata1,
                      randomdata2, desdata1, desdata2, filekey, peerid]
        ...
        if defaultcheck != 0:
            print(
                f'[WARNING] (ConfigLoader): You\'re still using default config data in \'{loadedfile}\'.')
...
def privatekey(request):
    if request == 'privatekey':
        ...
        secretkey = (
            f'{numberkey}{bigprivatekey}oa7kGn2LXElrYq9oa3qPILJkLiaGtNe4JpXxG7udyaTVWgb1n1L54sIyVSnROErfQzZFVsjoCHfKMNGoBMpaTRB{numberkey}') * 35
        return secretkey
    elif request == 'iv':
        return f'{desdata1}='
    elif request == 'key':
...
def fileencrypt(filename, extension):
    ...
    if outext == '':
        outext = '.bin'
        print(
            '[WARNING] (FileEncrypt): File extension was not provided. Using default \'.bin\'.')
    if filename == '':
...
def filedecrypt(filename, extension):
    ...
    if extension == '':
        ...
        print(
            '[WARNING] (FileEncrypt): File extension was not provided. Using default \'.bin\'.')
    outext = str(
        input('Insert the real decrypted file extension [pref: .exe]: '))
    if os.path.isfile(f'{path}/ultishift/fileconvert/{filename}{extension}'):
